...
<div class="bg-white rounded-lg shadow p-6 max-w-4xl">
    ...
    <form method="POST" action="<?= url('admin/events/update') ?>" enctype="multipart/form-data">
        <?php csrf_field(); ?>
        <input type="hidden" name="id" value="<?= (int)$item['id'] ?>">
        <input type="hidden" name="existing_image_url" value="<?= e($item['image_url'] ?? '') ?>">
        <input type="hidden" name="existing_content_image" value="<?= e($item['content_image'] ?? '') ?>">

        <!-- Title & Subtitle -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                <label class="block text-gray-700 text-sm font-bold mb-2">Event Title *</label>
                <input type="text" name="title" id="title-input" value="<?= e($item['title']) ?>"
            ...
            <div>
                ...
                <input type="text" name="subtitle" value="<?= e($item['subtitle']) ?>"
                    class="shadow border rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
        </div>
        ...
        <div class="mb-4">
            ...
            <div class="flex items-center gap-2">
                ...
                <input type="text" name="slug" id="slug-input" value="<?= e($item['slug'] ?? '') ?>"
            ...
            <p class="text-xs text-gray-400 mt-1">
                Current URL:
                <a href="<?= url('events/' . e($item['slug'] ?? '')) ?>" target="_blank"
                    class="text-blue-500 hover:underline">
                    <?= url('events/' . e($item['slug'] ?? '')) ?>
                </a>
            ...
            <?php if (!empty($item['image_url'])): ?>
                <div class="mb-3">
                    <img src="<?= asset('uploads/' . ltrim($item['image_url'], '/')) ?>"
                        class="h-32 rounded border object-cover" alt="Current thumbnail">
        ...
        <div class="mb-4 border rounded overflow-hidden">
            ...
            <div class="p-4 border-b">
                ...
                <p class="text-xs text-gray-400 mb-2">Optional. Clear the field to remove text from the event detail page.</p>
                <textarea name="content_text" rows="6"
                    class="shadow border rounded w-full py-2 px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="Enter event description or details here..."><?= e($item['content_text'] ?? '') ?></textarea>
            </div>
            <div class="p-4 bg-gray-50">
                ...
                <?php if (!empty($item['content_image'])): ?>
                    <div class="mb-3">
                        <img src="<?= asset('uploads/' . ltrim($item['content_image'], '/')) ?>"
                            class="max-h-48 rounded border object-cover" alt="Current detail image">
                        <p class="text-xs text-gray-400 mt-1">Current detail image</p>
                    </div>
                <?php endif; ?>
...
<script>
// Sanitise slug on blur (on edit page slug is pre-filled, don't auto-overwrite from title)
document.getElementById('slug-input').addEventListener('blur', function () {
