...
<form method="POST" action="<?= url('admin/partners/'.($item['id']?'update':'create')) ?>" enctype="multipart/form-data"><?php csrf_field(); ?>
...
<div class="mb-4"><label class="block text-gray-700 text-sm font-bold mb-2">Partner Name</label><input type="text" name="name" value="<?= e($item['name']) ?>" class="shadow border rounded w-full py-2 px-3" required></div>
<div class="mb-4"><label class="block text-gray-700 text-sm font-bold mb-2">Partner Logo</label>
...
<div class="flex gap-2"><button type="submit" class="bg-blue-600 text-white font-bold py-2 px-4 rounded">Save</button><a href="<?= url('admin/partners') ?>" class="bg-gray-500 text-white font-bold py-2 px-4 rounded">Cancel</a></div>
</form></div>
